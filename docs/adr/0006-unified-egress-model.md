...
- Adding the L7 proxy filtering (ADR not yet written; see ADR-0007) made
  the "is the proxy on?" question depend on a serde-level negation.
...
Before shipping the proxy/Wasm work as a first-class feature (ADR-0007),
the egress model needed an explicit, exhaustive enum so the serde round-
trip, the merge rules, the seccomp `connect()` enforcement, and the docs
all referenced the same three states.
...
- The proxy/Wasm work in ADR-0007 has a single load-bearing predicate
